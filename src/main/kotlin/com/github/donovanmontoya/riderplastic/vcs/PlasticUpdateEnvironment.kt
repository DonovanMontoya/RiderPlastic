...
class PlasticUpdateEnvironment(private val project: Project) : UpdateEnvironment {
    ...
    override fun updateDirectories(contentRoots: MutableCollection<FilePath>, updatedFiles: UpdatedFiles, progressIndicator: ProgressIndicator, force: MutableList<VcsException>, modifiableFiles: MutableList<FilePath>): com.intellij.openapi.vcs.update.UpdateSession {
        ...
        if (output == null) {
            force.add(VcsException("Failed to execute Plastic SCM checkout command."))
        } else if (output.exitCode != 0) {
            force.add(VcsException(output.stderr.ifBlank { output.stdout }.ifBlank { "Plastic SCM checkout failed with exit code ${output.exitCode}" }))
        }
        return object : com.intellij.openapi.vcs.update.UpdateSession {
            ...
            override fun getStatus(): com.intellij.openapi.vcs.update.UpdateSession.SessionStatus =
                if (force.isEmpty()) com.intellij.openapi.vcs.update.UpdateSession.SessionStatus.SUCCESS else com.intellij.openapi.vcs.update.UpdateSession.SessionStatus.ERROR
